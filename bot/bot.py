...
async def character(ctx, name: str):
    ...
    if nameCheck is None:
        ...
        note1name = await db.fetchval("SELECT note1name FROM characters WHERE nickname = $1;",)
        note2name = await db.fetchval("SELECT note2name FROM characters WHERE nickname = $1;",)
        note3name = await db.fetchval("SELECT note3name FROM characters WHERE nickname = $1;",)
        note4name = await db.fetchval("SELECT note4name FROM characters WHERE nickname = $1;",)
